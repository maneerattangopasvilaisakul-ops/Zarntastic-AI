...
คุณสามารถพิมพ์ปรึกษาโจทย์งาน หรือกดเลือกคำถามด่วนด้านล่างเพื่อรับคำแนะนำได้เลยครับ!`,
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl border border-stone-700/30 max-w-3xl w-full overflow-hidden flex flex-col h-[700px] max-h-[94vh]">
        
        {/* Header */}
        <div className="bg-gradient-to-r from-stone-900 via-indigo-950 to-purple-950 text-white p-3.5 sm:p-4 border-b border-indigo-800/40 shrink-0">
          <div className="flex items-center justify-between gap-3">
            
            {/* Title & Avatar */}
            <div className="flex items-center gap-2.5 sm:gap-3">
              <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl bg-gradient-to-tr from-purple-600 via-indigo-500 to-orange-400 p-0.5 flex items-center justify-center shadow-lg shadow-purple-500/20">
            ...
            <div className="flex items-center gap-1.5">
              <a
                ...
                title="ติดต่อ Admin ทาง LINE ID: zarn"
              >
              ...
              <button
                ...
                className="p-2 text-stone-300 hover:text-white rounded-xl hover:bg-white/10 transition-colors flex items-center gap-1 text-xs"
          ...
          <div className="mt-3 pt-2.5 border-t border-stone-800/80 flex flex-wrap items-center justify-between gap-2 text-xs">
            ...
            {/* Model Selector */}
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-stone-400 hidden sm:inline font-semibold">โมเดล AI:</span>
              <select
                id="select-gemini-model"
                value={selectedModel}
                onChange={(e) => setSelectedModel(e.target.value as GeminiModelType)}
                className="bg-stone-900 text-orange-300 text-[11px] font-semibold rounded-lg px-2.5 py-1 border border-stone-700 focus:outline-none focus:ring-1 focus:ring-orange-400 cursor-pointer"
              >
        ...
        {/* Message Thread */}
        <div className="flex-1 p-3.5 sm:p-5 overflow-y-auto space-y-4 bg-stone-50/70">
          {messages.map((m) => {
                ...
                <div className={`flex flex-col max-w-[88%] sm:max-w-[82%] ${isUser ? 'items-end' : 'items-start'}`}>
                  ...
                  >
                    {isUser ? (
                      ...
                      <div className="prose prose-sm max-w-none text-stone-800 prose-headings:font-bold prose-headings:text-stone-900 prose-headings:my-2 prose-p:my-1.5 prose-ul:my-1.5 prose-li:my-0.5 prose-strong:text-purple-900 prose-code:bg-purple-50 prose-code:text-purple-700 prose-code:px-1 prose-code:py-0.5 prose-code:rounded">
                    ...
                    {!isUser && (
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 transition-opacity absolute top-2 right-2 p-1 rounded-md bg-stone-100 hover:bg-stone-200 text-stone-600"
                        title="คัดลอกข้อความ"
                      >
                        {copiedId === m.id ? (
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 text-[10px] text-stone-400 mt-1.5 px-1">
                    <div className="flex items-center gap-1.5">
          ...
          {isLoading && (
            <div className="flex gap-2.5 items-center text-stone-500 text-xs">
              ...
              <div className="bg-white px-4 py-3 rounded-2xl border border-stone-200 shadow-xs flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-purple-500 animate-bounce" />
                <span className="w-2 h-2 rounded-full bg-indigo-500 animate-bounce [animation-delay:0.2s]" />
                <span className="w-2 h-2 rounded-full bg-orange-500 animate-bounce [animation-delay:0.4s]" />
                <span className="text-stone-600 font-medium text-xs ml-1">
        ...
        <div className="px-3 py-2 bg-stone-100/90 border-t border-stone-200 flex items-center gap-1.5 overflow-x-auto text-[11px] shrink-0 no-scrollbar">
          <span className="font-bold text-stone-600 shrink-0 text-[10px] uppercase tracking-wider flex items-center gap-1">
            <Sparkles className="w-3 h-3 text-purple-600" />
            คำถามด่วน:
          </span>
          {quickPrompts.map((item, idx) => (
            <button
              ...
              disabled={isLoading}
              className="bg-white hover:bg-purple-50 hover:border-purple-300 hover:text-purple-900 px-2.5 py-1 rounded-full border border-stone-200/90 text-stone-700 whitespace-nowrap transition-all cursor-pointer font-medium text-xs flex items-center gap-1 shrink-0 shadow-2xs"
            >
              <span>{item.icon}</span>
              <span>{item.text}</span>
            </button>
        ...
        >
          <input
            ...
            disabled={isLoading}
            className="flex-1 px-4 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:bg-white transition-all placeholder:text-stone-400"
          />
          <button
            type="submit"
